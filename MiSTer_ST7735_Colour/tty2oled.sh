...
senddata() {
  ...
  unset picfnam
  if [ "${USBMODE}" = "yes" ]; then						# Check the tty2xxx mode
    if [ -e "${picturefolder_pri}/${newcore}.col" ]; then			# Check for _pri pictures
       picfnam="${picturefolder_pri}/${newcore}.col"
	   dbug "Found COL Pri File"
	elif [ -e "${picturefolder_pri}/${newcore}.idx" ]; then			# Check for _pri pictures
       picfnam="${picturefolder_pri}/${newcore}.idx"
	   dbug "Found IDX Pri File"
	elif [ -e "${picturefolder_pri}/${newcore}.gsc" ]; then			# Check for _pri pictures
       picfnam="${picturefolder_pri}/${newcore}.gsc"
	   dbug "Found GSC Pri File"
    elif [ -e "${picturefolder_pri}/${newcore}.xbm" ]; then
       picfnam="${picturefolder_pri}/${newcore}.xbm"
	   dbug "Found XMB Pri File"
    else
      picfolders="gsc_us xbm_us col idx gsc xbm xbm_text"				# If no _pri picture found, try all the others
      [ "${USE_US_PICTURE}" = "no" ] && picfolders="${picfolders//gsc_us xbm_us/}"
	  [ "${USE_COL_PICTURE}" = "no" ] && picfolders="${picfolders//col/}"
	  [ "${USE_IDX_PICTURE}" = "no" ] && picfolders="${picfolders//idx/}"
