...
//uncomment to use test Amiga logo from logo.h (for testing display routines)
//#define TESTDISPLAY
...
void loop(void) {
  ...
  if (strcmp(newCore,oldCore)) {                                    // Proceed only if Core Name changed
    ...
    else if (strcmp(newCore,"att")==0)          {}                                 // Do nothing but needed to get an "CORECHANGE" working via USB
    else if (strcmp(newCore,"CORECHANGE")==0) {                                    // Annoucement to receive Data via USB Serial from the MiSTer
      #ifdef USE_NODEMCU
...
/*size_t my_readSerialPic(void)
{
  ...
  inchar=Serial.read();
  //Serial.printf("Serial pointer=%d Char=%c \n",index,inchar);
  while ((index<2048) && (Serial.available()>0))
  {
    imageBits[index]=inchar;
    index++;
    delay(10);     //give the serial buffer time to fill up!
    inchar=Serial.read();
    //Serial.printf("Serial pointer=%d Char=%c \n",index,inchar);
  }
  //newCore[index]='\0';
...
int usb2oled_readndrawlogo(int effect) {
  const int logoBytes = 2048;//DispWidth * DispHeight / 8; // Make it more universal, here 2048
  int logoByte;
  unsigned char logoByteValue;
  int w,x,y,x2;
  unsigned char *imageBits;
//  unsigned char *logoBits;
  size_t bytesread;

  #ifdef TESTDISPLAY
    imageBits=(unsigned char *)amiga_logo;
  #else
    imageBits = (unsigned char *) malloc(logoBytes);    // Reserve Memory for Picture-Data
  #endif
  
  

  #ifdef TESTDISPLAY
    //ditch serial buffer to avoid being read as another core
    delay(1000);
    while (Serial.available())
    {
      logoByteValue=Serial.read();
    }
  #else
    #ifdef XDEBUG
      Serial.printf("Attempting to read %d bytes from Serial\n",logoBytes);
    #endif

    //delay(1000);
  
    bytesread=Serial.readBytes(imageBits,logoBytes);             // Read 2048 Bytes from Serial
    //bytesread=my_readSerialPic();

    #ifdef XDEBUG
      Serial.printf("Successfully read %d bytes from Serial\n",bytesread);

      for (int j=0;j<bytesread;j++){
        Serial.printf("Byte at loc %d is %xhh\n",j,imageBits[j]);
      }
    #endif
   #endif
  ...
  //imageBits=(unsigned char*)&amiga_logo[0];

  halfsize=1;                                       //for USB version assume all pics are 256 pixels wide!
  u8g2_uint_t imageHeight=64;
  u8g2_uint_t imageWidth=256;
